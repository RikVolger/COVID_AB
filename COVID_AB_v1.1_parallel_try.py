...
print('Gonna be runnin\' this baby on {} cores\n---------------\n'.format(num_cores))
...
class person:
  ...
  def __init__(self, ID, n_contacts, infected, infective, day_inf, non_inf_dur, inf_dur, 
    ...
    # person identification, used throughout the models - advised to count upwards from 0 - integer
    self.ID                         = ID
  ...
  def infect(self, population_yesterday, contact_matrix, today):
    '''
    get infected by friends according to the contact matrix
    '''
    contacts = np.flatnonzero(contact_matrix[:, self.ID])
    # contacts = np.flatnonzero(contacts_row)
    # contacts = contacts[0]
    # infect others
    for ID in contacts:
...
def update(p, population_yesterday, today, contact_matrix):
  if p.dead:
    return p
  # check for infectivity at p from yesterday
  if population_yesterday[p.ID].infected:
    # if past the duration of infection, either die or immunize
    if day - p.day_of_infection > p.infection_duration:
      if p.fate == 'die':
        p.die()
        # print('Person {} died on the {}th day'.format(p.ID, day))
      elif p.fate == 'immunize':
        p.immunize()
        # print('Person {} was cured on the {}th day'.format(p.ID, day))
    # otherwise, infect contacts
  elif not p.immune:
    p.infect(population_yesterday, contact_matrix, today)
    # print('Person {} infected his friends on the {}th day'.format(p.ID, day))
  elif population_yesterday[p.ID].infected:
    if day - p.day_of_infection > p.non_infective_period:
      p.infective = True
...
with Parallel(n_jobs=num_cores) as parallel:
	for day in range(1, n_days):
	  ...
	  population_start_today = copy.deepcopy(population_yesterday)
	  print(day)
	  # gather data on infectees etc. this day
	  # print(delayed(update(p, population_yesterday, day, contact_matrix)) for p in population_start_today)
	  if __name__ == "__main__":
	    population_today = parallel(delayed(update)(p, population_yesterday, day, contact_matrix) for p in population_start_today)
	  # for p in population_start_today:
	  #   # update(self, population_today, population_yesterday, day, contact_matrix):
	  #   p.update(population_yesterday, day, contact_matrix)
