...
class person:
  ...
  def __init__(self, ID, n_contacts, infected, infective, day_inf, inf_dur, population_size):
    self.ID                         = ID
  ...
  def update(self, population_today, population_yesterday, day, contact_matrix):
    if self.dead:
      return
    ...
    if population_yesterday[self.ID].infective:
      ...
      # otherwise, infect contacts
        ...
        self.infect(population_today, contact_matrix, day)
  ...
  def infect(self, population_today, contact_matrix, day):
    '''
    ...
    for ID in contacts:
      ...
      if not infectee.infected and not infectee.immune:
        ...
        infectee.infective = True
        infectee.day_of_infection = day
...
def fill_contact_matrix(ID, n_contacts, population_size):
  ...
  if n_current_friends < n_contacts:
    ...
    new_friends = random.choices(available_contacts, k=int(n_new_friends))
  ...
  try:
    for ID in range(population_size):
      if ID in ID_infected_0:
        ...
        infective = True
        day_inf = 0
        population_initial.append(person(ID, n_contacts, infected, infective, day_inf, inf_dur, population_size))
      else:
        infected = False
        infective = False
        day_inf = 0
